...
## PrizePool / PrizeItem (`types/prize-pool.ts`, `data/prize-pools.ts`)

Backs **both** Wheel Spin and Mystery Eggs — `kind: "wheel" | "egg"` distinguishes them.
`quantity` is set for egg tiers (1/5/10/15) and undefined for the wheel. Each `PrizeItem` has a
`weight` (relative odds, admin-editable in a future dashboard without touching code) and a
`rarity` (`common | uncommon | rare | jackpot`, drives the `RevealPanel` badge color/label).
`pickWeighted()` in `lib/utils.ts` does the actual weighted-random selection client-side; there is
no server-side odds enforcement in Phase 1 (see [[07-roadmap]] — that's a Phase 2 concern once
there's a real backend to trust for it).
